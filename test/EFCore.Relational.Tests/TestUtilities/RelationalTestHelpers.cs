...
";
        ...
        public static void AssertBaseline(params string[] expected)
        {
            ...
            try
            {
                Assert.Equal(expected[0], sqlStatements[0]);

                foreach (var expectedFragment in expected.Skip(1))
                {
            ...
            catch
            {
                ...
                var lineNumber = methodCallLine.Substring(methodCallLine.LastIndexOf("line", StringComparison.Ordinal));
